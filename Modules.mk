...
ifeq ($(strip $(VERBOSE)),1)
	...
	@$(VCFGMSG) 'CXX:' '$(CXX)'
	@$(VCFGMSG) 'AR:' '$(AR)'
	@$(VCFGMSG) 'MODULE_OPTS:' '$(MODULE_OPTS_$*)'
endif
	$(V)$(MAKE) --directory='$(MODULES_PATH_$*)' --file='$(MAKE_INC_PATH)/ModulesMakefile.mk' 'SUBTARGET_NAME=$*' 'SUBTARGET_PATH=$(MODULES_PATH_$*)' \
				'MAKE_INC_PATH=$(MAKE_INC_PATH)' $(MODULE_OPTS_$*) 'BUILD_DIR=$(BUILD_DIR)' 'CPU=$(CPU)' 'CFLAGS=$(CFLAGS)' 'CPPFLAGS=$(CPPFLAGS)' 'ARFLAGS=$(ARFLAGS)' \
				'USE_ARDUINO_MAIN=$(USE_ARDUINO_MAIN)' 'CORE_SKIP_NEW_O=$(CORE_SKIP_NEW_O)' 'CXXFLAGS=$(CXXFLAGS)' 'LDFLAGS=$(LDFLAGS)' 'ASMFLAGS=$(ASMFLAGS)' \
				'STARTUP_MAKEFILE=$(STARTUP_MAKEFILE)' 'CPU_BASE_TARGET=$(CPU_BASE_TARGET)' 'CC=$(CC)' 'CXX=$(CXX)' 'AR=$(AR)' 'V=$(V)' 'MSG=$(MSG)' $(MAKEFLAGS) all
...
lib%-$(CPU).a.modulecfg:
	$(V)$(MAKE) --directory='$(MODULES_PATH_$*)' --file='$(MAKE_INC_PATH)/ModulesMakefile.mk' 'SUBTARGET_NAME=$*' 'SUBTARGET_PATH=$(MODULES_PATH_$*)' \
 					'MAKE_INC_PATH=$(MAKE_INC_PATH)' 'BUILD_DIR=$(BUILD_DIR)' 'CPU=$(CPU)' 'CFLAGS=$(CFLAGS)' 'CPPFLAGS=$(CPPFLAGS)' 'ARFLAGS=$(ARFLAGS)' \
 					'USE_ARDUINO_MAIN=$(USE_ARDUINO_MAIN)' 'CORE_SKIP_NEW_O=$(CORE_SKIP_NEW_O)' 'CXXFLAGS=$(CXXFLAGS)' 'LDFLAGS=$(LDFLAGS)' 'ASMFLAGS=$(ASMFLAGS)' \
 					'STARTUP_MAKEFILE=$(STARTUP_MAKEFILE)' 'CPU_BASE_TARGET=$(CPU_BASE_TARGET)' 'CC=$(CC)' 'CXX=$(CXX)' 'AR=$(AR)' 'V=$(V)' 'MSG=$(MSG)' cfg
