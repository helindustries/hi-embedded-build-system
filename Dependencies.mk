...
ifeq ($(strip $(VERBOSE)),1)
	...
	$(V)$(MAKE) --directory="$(DEPENDENCY_PATH_$*)" --file="$(DEPENDENCY_PATH_$*)/Makefile" $(DEPENDENCY_OPTS_$*) 'STARTUP_MAKEFILE=$(STARTUP_MAKEFILE)' \
 					'MAKE_INC_PATH=$(MAKE_INC_PATH)' 'CPU_TARGET=$*' 'BUILD_DIR=$(BUILD_DIR)/$*' 'CPU_DEVICE=$(CPU_DEVICE)' \
 					'VERBOSE=$(VERBOSE)' 'DEBUG=$(DEBUG)' 'NO_GATEWARE_DEPS=$(NO_GATEWARE_DEPS)' 'NO_TOOLS_DEPS=$(NO_TOOLS_DEPS)' \
 					'CPU_BASE_TARGET=$(CPU_BASE_TARGET)' 'NO_TESTS_DEPS=$(NO_TESTS_DEPS)' 'NO_GATEWARE_UPLOAD=$(NO_GATEWARE_UPLOAD)' $(MAKECMDGOALS)
# 'CPU_EXEC_OFFSET=$(CPU_EXEC_OFFSET)' 'OPTIMIZE=$(OPTIMIZE)' 'CPU=$(CPU)' 'CPU_SPEED=$(CPU_SPEED)' 'BUS_SPEED=$(BUS_SPEED)' 'CPUARCH=$(CPUARCH)' \
# 					'CPU_CPPFLAGS=$(CPU_CPPFLAGS)' 'CPU_LDFLAGS=$(CPU_LDFLAGS)' 'SERIAL_ID=$(SERIAL_ID)' 'CPU_DEVICE_OPTS=$(CPU_DEVICE_OPTS)' \
# 					'CORE_LIB_PATH=$(CORE_LIB_PATH)' 'CORE_PATH=$(CORE_PATH)' $(MAKEFLAGS) 'CPU_TARGET=$*'
...
ifeq ($(strip $(VERBOSE)),1)
	...
	$(V)$(MAKE) --directory="$(DEPENDENCY_PATH_$*)" --file="$(DEPENDENCY_PATH_$*)/Makefile" $(DEPENDENCY_OPTS_$*) 'STARTUP_MAKEFILE=$(STARTUP_MAKEFILE)' \
 					'MAKE_INC_PATH=$(MAKE_INC_PATH)' 'BUILD_DIR=$(BUILD_DIR)/$*' 'CPU_TARGET=$*' 'CPU_DEVICE=$(CPU_DEVICE)' \
 					'VERBOSE=$(VERBOSE)' 'DEBUG=$(DEBUG)' 'NO_GATEWARE_DEPS=$(NO_GATEWARE_DEPS)' 'NO_TOOLS_DEPS=$(NO_TOOLS_DEPS)' \
 					'CPU_BASE_TARGET=$(CPU_BASE_TARGET)' 'NO_TESTS_DEPS=$(NO_TESTS_DEPS)' 'NO_GATEWARE_UPLOAD=$(NO_GATEWARE_UPLOAD)' $(MAKECMDGOALS)
# 'CPU_EXEC_OFFSET=$(CPU_EXEC_OFFSET)' 'OPTIMIZE=$(OPTIMIZE)'  'CPU=$(CPU)' 'CPU_SPEED=$(CPU_SPEED)' 'BUS_SPEED=$(BUS_SPEED)' 'CPUARCH=$(CPUARCH)' \
# 					'CPU_CPPFLAGS=$(CPU_CPPFLAGS)' 'CPU_LDFLAGS=$(CPU_LDFLAGS)' 'SERIAL_ID=$(SERIAL_ID)' 'CPU_DEVICE_OPTS=$(CPU_DEVICE_OPTS)' \
...
ifeq ($(strip $(VERBOSE)),1)
	...
	$(V)$(MAKE) --directory="$(DEPENDENCY_PATH_$*)" --file="$(DEPENDENCY_PATH_$*)/Makefile" $(DEPENDENCY_OPTS_$*) 'STARTUP_MAKEFILE=$(STARTUP_MAKEFILE)' \
 					'MAKE_INC_PATH=$(MAKE_INC_PATH)' 'CPU_DEVICE=$(PLATFORM_ID)' 'BUILD_DIR=$(BUILD_DIR)/$*' 'OPTIMIZE=$(OPTIMIZE)' 'VERBOSE=$(VERBOSE)' 'DEBUG=$(DEBUG)' \
