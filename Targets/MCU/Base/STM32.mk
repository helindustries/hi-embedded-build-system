...
ifneq ($(strip $(NO_FIRMWARE_UPLOAD)),yes)
	...
	$(V)$(STM32_CUBEPROG) $(STM32_UPLOAD_TYPE_ID) "$<" $(STM32_VECTOR_TABLE_OFFSET) && echo "$(MCU_BOARD_PORT)" > "$(BUILD_DIR)/.last_esp32_port" && touch "$@"
	    #$(PROCESS_OUTPUT)
endif

upload_stm32: $(BUILD_DIR)/$(MCU_TARGET)-$(MCU).bin.upload_stm32.timestamp | silent
	@
