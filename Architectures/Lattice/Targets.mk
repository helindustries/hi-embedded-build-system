...
FPGA_DEVICE_ID_UPPER := $(shell echo "$(FPGA_DEVICE_ID)" | tr '[:lower:]' '[:upper:]')
...
clean_lattice: clean-modelsim
	...
	$(V)rm -f "$(FPGA_TARGET).prf" "$(FPGA_TARGET).synproj" "$(FPGA_TARGET).ldf" "$(FPGA_TARGET).ngd" "$(FPGA_TARGET)_map.ncd" "$(FPGA_TARGET).ncd" "$(FPGA_TARGET).$(FPGA_DEVICE).bit" "$(FPGA_TARGET).$(FPGA_DEVICE).bit.lattice.svf" "$(FPGA_TARGET).wcfg"
