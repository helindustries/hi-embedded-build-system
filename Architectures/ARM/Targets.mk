...
clean_arm:
	...
	$(V)$(RM) "$(CPU_TARGET)-$(CPU)$(CPU_BINARY_EXT)" "$(CPU_TARGET)-$(CPU).hex" "$(CPU_TARGET)-$(CPU).eep" "$(CPU_TARGET)-$(CPU).sym" "$(CPU_TARGET)-$(CPU).lst" "$(CPU_TARGET)-$(CPU).post" "$(CPU_TARGET)-$(CPU).zip"
...
ifneq ($(strip $(NO_FIRMWARE_UPLOAD)), yes)
	...
	$(V)"$(OPENOCD)" $(OPENOCD_DEBUG) -s "$(OPENOCD_CFG_DIR)" -f "$(OPENOCD_CFG_DIR)/$(CPU_DEBUG_ADAPTER).cfg" -f "$(MAKE_INC_PATH)/Devices/CPU/$(CPU_DEVICE).ocd.cfg" -c "capture 'reset halt; flash write_image erase $(CPU_TARGET)-$(CPU).hex'" -c "shutdown 0" $(ARM_CREATE_TIMESTAMP)
endif
