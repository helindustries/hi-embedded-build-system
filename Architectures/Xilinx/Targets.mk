...
%.ngd: %.ngc %.$(FPGA_CONSTRAINTS) $(FPGA_TARGET_DEPS)
	...
	@$(FMSG) "INFO:Building $@"
	$(V)$(XILINX_ISE_WINE) $(XILINX_ISE_NGDBUILD) -intstyle $(INTSTYLE) -dd _ngo -nt timestamp -uc "$*.$(FPGA_CONSTRAINTS)" $(NGDBUILD_ARGS) -p $(FPGA_DEVICE_ID) "$<" "$@" $(PROCESS_OUTPUT)
...
%_map.ncd: %.ngd $(FPGA_TARGET_DEPS)
	...
	$(V)$(XILINX_ISE_WINE) $(XILINX_ISE_MAP) -intstyle $(INTSTYLE) -p $(FPGA_DEVICE_ID) -w -logic_opt off -ol high -t 1 -xt 0 -register_duplication off -r 4 -global_opt off -mt off -ir off -pr off -lc off -power off -o "$@" "$<" "$*.pcf" $(PROCESS_OUTPUT)
...
ifneq ($(strip $(NO_GATEWARE_UPLOAD)),yes)
	...
	$(V)set -o pipefail && "$(OPENOCD)" $(OPENOCD_DEBUG) -s "$(OPENOCD_CFG_DIR)" -f "$(OPENOCD_CFG_DIR)/$(FPGA_DEBUG_ADAPTER).cfg" -f "$(MAKE_INC_PATH)/Devices/FPGA/$(FPGA_DEVICE).ocd.cfg" -c "init" -c "scan_chain" -c "svf $< -ignore_error" -c "shutdown" $(PROCESS_OUTPUT) $(XILINX_CREATE_TIMESTAMP)
endif
